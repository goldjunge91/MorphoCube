...
export interface IStorage {
  ...
  getParametersByUserId(userId: number): Promise<Parameter[]>;
  ...
  getSharedAccessByUserAndBox(userId: number, morphBoxId: number): Promise<SharedAccess | undefined>;
...
export class MemStorage implements IStorage {
  ...
  async getParametersByUserId(userId: number): Promise<Parameter[]> {
    return Array.from(this.parameters.values())
      .filter(parameter => parameter.userId === userId);
  }
  ...
  async getAttributesByParameterId(parameterId: number): Promise<Attribute[]> {
    return Array.from(this.attributes.values())
      .filter(attribute => attribute.parameterId === parameterId);
  }
  ...
  async getMorphBoxesByUserId(userId: number): Promise<MorphBox[]> {
    return Array.from(this.morphBoxes.values())
      .filter(morphBox => morphBox.userId === userId);
  }
  ...
  async getSharedAccessByUserAndBox(userId: number, morphBoxId: number): Promise<SharedAccess | undefined> {
    return Array.from(this.sharedAccesses.values())
      .find(access => access.userId === userId && access.morphBoxId === morphBoxId);
  }
