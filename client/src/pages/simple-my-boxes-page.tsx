...
export default function SimpleMyBoxesPage() {
  ...
  // Fetch specific box by ID (mock implementation)
  const fetchBoxById = async (id: number) => {
    try {
      // Simulate API call
      setIsLoading(true);
      setTimeout(() => {
        const box = morphBoxes.find(box => box.id === id);
        if (box) {
          setEditingBox(box);
        } else {
  ...
  const handleBoxSaved = (box: MorphBox) => {
    toast({
      title: "Box saved",
      description: "The morphological box has been saved successfully.",
    });
    navigate("/my-boxes");
  };
  ...
  if (editingBox || isCreating) {
    return (
      <SimpleLayout title={editingBox ? `Editing: ${editingBox.title}` : "Create New Box"}>
  ...
  return (
    <SimpleLayout title="My Morphological Boxes">
